                                 ...
                                 const Dictionary& settings ) EXCEPT ( ConfigError )
{
  ...
  if ( settings.has( ENABLE_NEXT_EXPECTED_MSG_SEQ_NUM ) )
    pSession->setEnableNextExpectedMsgSeqNum( settings.getBool( ENABLE_NEXT_EXPECTED_MSG_SEQ_NUM ) );
  if ( settings.has( MAX_MESSAGES_IN_RESEND_REQUEST ) )
                                                                     ...
                                                                     const std::string& settingsKey) EXCEPT (ConfigError)
{
  ...
  if( settings.has( VALIDATE_FIELDS_FORMAT ) )
    pCopyOfDD->checkFieldsFormat( settings.getBool( VALIDATE_FIELDS_FORMAT ) );
