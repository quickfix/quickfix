#ifndef FIX50SP2_MASSORDER_H
#define FIX50SP2_MASSORDER_H
...
namespace FIX50SP2
{
  ...
  class MassOrder : public Message
  {
  public:
    MassOrder() : Message(MsgType()) {}
    MassOrder(const FIX::Message& m) : Message(m) {}
    MassOrder(const Message& m) : Message(m) {}
    MassOrder(const MassOrder&) = default;
    MassOrder(MassOrder&&) = default;
    MassOrder& operator=(const MassOrder&) = default;
    MassOrder& operator=(MassOrder&&) = default;
    static FIX::MsgType MsgType() { return FIX::MsgType("DJ"); }

    MassOrder(
      const FIX::MassOrderRequestID& aMassOrderRequestID )
    : Message(MsgType())
    {
      set(aMassOrderRequestID);
    }
