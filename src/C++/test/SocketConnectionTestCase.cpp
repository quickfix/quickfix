...
SUITE(SocketConnectionTests)
{
  ...
  class TestSocketReceiver : public SocketConnection::SocketReceiver {
  public:
    TestSocketReceiver() {};
    virtual ~TestSocketReceiver(){};

    virtual ssize_t receive( int s, char* buf, size_t length ) {
      strcpy(buf,currentBuffer);
      if(invalidMessage) {
        throw InvalidMessage();
      }
      if(socketRecvFailed){
        throw SocketRecvFailed("socketRecvFailed");
      }
      return sizeof(currentBuffer);
    }
    ...
    char currentBuffer[4096];
    int selectResult = -1;
    bool invalidMessage = false;
    bool socketRecvFailed = false;
  ...
  TEST_FIXTURE(TestSessionConnection, SignalSocket_MessageQueueHasOneMessage_MonitorSignaled)
  {
    ...
    CHECK_EQUAL(socket, monitor.signaledSocket);
  }

  TEST_FIXTURE(TestSessionConnection, UnsignalSocket_MessageQueueHasZeroMessages_MonitorUnSignaled)
  {
    pSocketConnection->unsignal();
    CHECK_EQUAL(socket, monitor.unsignalSocket);
  }

  TEST_FIXTURE(TestSessionConnection, DisconnectedSession_MonitorDropSocketSignaled)
  {
    Session* session = getSession();
    session->disconnect();
    CHECK_EQUAL(socket, monitor.dropSocket);
  }
  ...
  TEST_FIXTURE(TestSessionConnection, ReadSocket_SuccessfulRead)
  {
    SocketConnector connector;
    std::shared_ptr<TestSocketReceiver> rec = std::make_shared<TestSocketReceiver>(TestSocketReceiver());
    FIX42::Logon logon;
    strcpy(rec->currentBuffer, logon.toString(FIELD::BeginString, FIELD::BodyLength, FIELD::CheckSum).c_str());
    pSocketConnection->setSocketReciever(rec);


    CHECK(pSocketConnection->read(connector));
  }

  TEST_FIXTURE(TestSessionConnection, ReadSocket_SocketTimeOut_ReadFailed)
  {
    std::set<SessionID> sessions;
    pSocketConnection.reset(new SocketConnection( socket, sessions, &monitor ));
    ...
    CHECK(!pSocketConnection->read(*acceptor, server));
  }

  TEST_FIXTURE(TestSessionConnection, ReadSocket_InvalidSession_ReadFailed)
  {
    std::set<SessionID> sessions;
    pSocketConnection.reset(new SocketConnection( socket, sessions, &monitor ));
  ...
  TEST_FIXTURE(TestSessionConnection, ReadSocket_SocketRecvFailed_ReadFailed)
  {

    SocketConnector connector;
    std::shared_ptr<TestSocketReceiver> rec = std::make_shared<TestSocketReceiver>(TestSocketReceiver());
    rec->selectResult = 1;
    rec->socketRecvFailed = true;
    pSocketConnection->setSocketReciever(rec);
  ...
  TEST_FIXTURE(TestSessionConnection, ReadSocket_NoSessionInvalidMessage_SuccessfulRead)
  {
    ...
    rec->selectResult = 1;
    strcpy(rec->currentBuffer, logon.toString(FIELD::BeginString, FIELD::BodyLength, FIELD::CheckSum).c_str());
    initiatorSession->nextThrowInvalidMsg=true;
    pSocketConnection->setSocketReciever(rec);
  ...
  TEST_FIXTURE(TestSessionConnection, ReadSocket_SessionExistsInvalidMessage_ReadFailed)
  {
    std::set<SessionID> sessions;
    sessions.insert(acceptorSessionID);
    sessions.insert(initiatorSessionID);
    Session::registerSession( acceptorSessionID );
    Session::registerSession( initiatorSessionID );
    ...
    std::shared_ptr<TestSocketReceiver> rec = std::make_shared<TestSocketReceiver>(TestSocketReceiver());
    FIX42::Logon logon;
    logon.getHeader().set(BeginString(acceptorSessionID.getBeginString()));
    logon.getHeader().set(SenderCompID(acceptorSessionID.getSenderCompID()));
    logon.getHeader().set(TargetCompID(acceptorSessionID.getTargetCompID()));

    rec->selectResult = 1;
    strcpy(rec->currentBuffer, logon.toString(FIELD::BeginString, FIELD::BodyLength, FIELD::CheckSum).c_str());
    acceptorSession->nextThrowInvalidMsg=true;
    pSocketConnection->setSocketReciever(rec);
  ...
  TEST_FIXTURE(TestSessionConnection, ReadSocket_SessionNotRegistered_ReadFailed)
  {
    std::set<SessionID> sessions;
    pSocketConnection.reset(new SocketConnection( socket, sessions, &monitor ));
    Session::registerSession(initiatorSessionID);
  ...
  TEST_FIXTURE(TestSessionConnection, ReadSocket_MessageUnParseable_ReadFailed)
  {
    std::set<SessionID> sessions;
    pSocketConnection.reset(new SocketConnection( socket, sessions, &monitor ));
    ...
    rec->selectResult = 1;
    strcpy(rec->currentBuffer, "8=FIX.4.2\0019=2A\001");
    pSocketConnection->setSocketReciever(rec);
    CHECK(!pSocketConnection->read(*acceptor, server));
  }
