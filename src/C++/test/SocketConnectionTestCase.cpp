...
class TestSocketApplication : public FIX::NullApplication
{
public:
  TestSocketApplication() : m_count(0) {}
...
struct BaseSocketConnection : public ::testing::Test
{
  ...
  TestSocketApplication application;
  ...
  socket_handle socket = 101; // invalid socket handle for error checking
};
...
TEST_F(NonTestSessionConnection, socketTimeout_SessionIsNotNull)
{
...
TEST_F(TestSessionConnection, ReadSocket_SessionMissing_NoMessagesRead)
{
  std::set<SessionID> sessions;
  pSocketConnection.reset(new SocketConnection(socket, sessions, &monitor));

  SocketConnector connector;
  ASSERT_TRUE(!pSocketConnection->read(connector));
}

TEST_F(TestSessionConnection, ReadSocket_SocketException_ReadFailed)
{
