...
{
  const int order[] = {3, 1, 2};
  FieldMap fieldMap(order);
  fieldMap.setField(1, "account");
  fieldMap.setField(2, "adv_id");
  fieldMap.setField(3, "adv_ref_id");

  std::vector<int> actual_order;
  for(FieldMap::iterator itr = fieldMap.begin(); itr != fieldMap.end(); itr++) {
    actual_order.push_back(itr->getTag());
  }

  std::vector<int> expected_order = {3,1,2};
  CHECK_ARRAY_EQUAL(expected_order, expected_order, 3);
}
